  ...
  useEffect(() => {
    async function getIds() {
      const res = await fetch(
        "https://api.byd-film.de/v1/company/byd-film/kollektionen/latest",
      );
      const allinfo = [];
    ...
    <div className="collection-display">
      {items.map((item, info) => (
        <div
          ...
          style={{
            background: `linear-gradient(180deg, rgba(0, 0, 0, 0.1) 0%, rgba(0, 0, 0, 0.2) 100%), url("http://pictures.byd-film.de/kollektionen/${item.id}/thumbnail.jpg")  no-repeat center /cover`,
          }}
          key={info}
        >
